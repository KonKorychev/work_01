...
Далее рассмотрим как распределились количественные признаки по отношению к целевой переменной. Насколько разные распределения признаков **transaction_date_int** и **transaction_amount** для обычных и подозрительных операций. Выдвинем нулевую гипотезу о том, что матожидания в обеих выборках равны. Гипотезу проверим с помощью доверительных интервалов и критерия Вилкоксона (Манна-Уитни).
